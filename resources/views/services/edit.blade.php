@extends('template.main', ['activePage' => 'services', 'titlePage' => __('Editar servicio')])

@section('content')
  <form class="form" method="POST" action="{{ route('services.update', ['id' => $service->id]) }}">
    @method('put')
    @csrf
    <div class="bmd-form-group{{ $errors->has('code') ? ' has-danger' : '' }}">
      <div class="input-group">
        ...
        <input type="text" name="code" id="code" class="form-control" placeholder="{{ __('Código...') }}" value="{{ $service->code }}">
      </div>
      @if ($errors->has('code'))
    ...
    <div class="bmd-form-group{{ $errors->has('name') ? ' has-danger' : '' }} mt-3">
      <div class="input-group">
        ...
        <input type="text" name="name" class="form-control" placeholder="{{ __('Nombre del servicio...') }}" value="{{ $service->name }}" required>
      </div>
      @if ($errors->has('name'))
    ...
    <div class="bmd-form-group{{ $errors->has('description') ? ' has-danger' : '' }} mt-3">
      <div class="input-group">
        ...
        <input type="text" name="description" class="form-control" placeholder="{{ __('Descripción del producto...') }}" value="{{ $service->description }}" required>
      </div>
    ...
    <div class="bmd-form-group{{ $errors->has('price') ? ' has-danger' : '' }} mt-3">
      <div class="input-group">
        ...
        <input type="number" step="0.01" name="price" id="price" class="form-control" placeholder="{{ __('Precio...') }}" value="{{ $service->price }}" required>
      </div>
      @if ($errors->has('price'))
