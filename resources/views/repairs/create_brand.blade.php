@extends('template.main', ['activePage' => 'repair-brands', 'titlePage' => __('Nueva marca')])

@section('content')
  <form class="form" method="POST" action="{{ route('repairs.brands.store') }}">
    @csrf
    <div class="bmd-form-group{{ $errors->has('name') ? ' has-danger' : '' }} mt-3">
      <div class="input-group">
        ...
        <input type="text" name="name" class="form-control" placeholder="{{ __('Nombre de la marca...') }}" value="{{ old('name') }}" required>
      </div>
      @if ($errors->has('name'))
